...
router.beforeEach((to, from, next) => {
  ...
  if ($('.st-button-menu-mobile').length > 0) {
    if ($('.modal-menu-mobile').hasClass('st_dialogIsOpen')) {
      $('.st-button-menu-mobile').trigger('touchend')
    }
  }
  next()
});
  ...
  methods: {
    fetchData() {
      ...
      $.ajax({
        ...
        success(response) {
          if (_this.nroCalls > 0) {
            if (response.title !== undefined && response.meta !== undefined) {
              _this.title = response.title
              _this.meta = response.meta
            }
          }
          if (response) {
            store.commit('pushResponse', response)
          }
